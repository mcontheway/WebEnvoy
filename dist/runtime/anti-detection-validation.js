...
const assertRequiredSourceValidationSignals = (signals) => {
    for (const requiredScope of XHS_CLOSEOUT_REQUIRED_VALIDATION_SCOPES) {
        ...
        throw new RuntimeStoreError("ERR_RUNTIME_STORE_INVALID_INPUT", `XHS closeout validation source is missing required ${requiredScope.targetFrRef}/${requiredScope.validationScope} evidence`);
    }
};
...
const signalFromSourceSample = (sample, expectedScope) => {
    ...
    if (!signal || sample.validation_scope !== expectedScope) {
        throw new RuntimeStoreError("ERR_RUNTIME_STORE_INVALID_INPUT", "XHS closeout validation source sample signal is invalid");
    }
    return signal;
};
const sourceSampleForRequiredScope = (samples, targetFrRef, validationScope) => {
    const sample = samples.find((candidate) => candidate.target_fr_ref === targetFrRef &&
        candidate.validation_scope === validationScope) ?? null;
    if (!sample) {
        throw new RuntimeStoreError("ERR_RUNTIME_STORE_INVALID_INPUT", "XHS closeout validation source samples are incomplete");
    }
    return sample;
};
const closeoutSignalsFromSourceSamples = (samples) => ({
    layer1_consistency: signalFromSourceSample(sourceSampleForRequiredScope(samples, "FR-0012", "layer1_consistency"), "layer1_consistency"),
    layer2_interaction: signalFromSourceSample(sourceSampleForRequiredScope(samples, "FR-0013", "layer2_interaction"), "layer2_interaction"),
    layer3_session_rhythm: signalFromSourceSample(sourceSampleForRequiredScope(samples, "FR-0014", "layer3_session_rhythm"), "layer3_session_rhythm")
});
export const persistXhsCloseoutValidationSourceSamples = async (input) => {
    ...
    await input.store.runInTransaction(async () => {
        for (const requiredScope of XHS_CLOSEOUT_REQUIRED_VALIDATION_SCOPES) {
            ...
            const sourceSample = sourceSampleForRequiredScope(input.sourceSamples, requiredScope.targetFrRef, requiredScope.validationScope);
            if (sourceSample.run_id !== input.sourceRunId ||
                ...
                throw new RuntimeStoreError("ERR_RUNTIME_STORE_INVALID_INPUT", "XHS closeout validation source sample scope does not match requested baseline");
            }
            const sourceRequest = await input.store.getAntiDetectionValidationRequest(sourceSample.request_ref);
            if (!sourceRequest ||
                ...
                throw new RuntimeStoreError("ERR_RUNTIME_STORE_INVALID_INPUT", "XHS closeout validation source sample request does not match requested baseline");
            }
