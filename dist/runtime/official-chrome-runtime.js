...
const isRuntimeBootstrapPendingCode = (code) => code === "ERR_RUNTIME_BOOTSTRAP_NOT_DELIVERED" ||
    code === "ERR_RUNTIME_BOOTSTRAP_ACK_TIMEOUT";
const isRuntimeBootstrapFailureCode = (code) => code === "ERR_RUNTIME_BOOTSTRAP_ACK_STALE" ||
...
const readOfficialChromeRuntimeReadinessViaBridge = async (input) => {
    ...
    const hasTransportState = payload?.transport_state === "disconnected" ||
        payload?.transport_state === "ready" ||
        payload?.transport_state === "not_connected";
    const hasBootstrapState = payload?.bootstrap_state === "not_started" ||
    ...
    if (!hasTransportState && !hasBootstrapState) {
        throw new CliError("ERR_RUNTIME_UNAVAILABLE", "official Chrome runtime readiness 缺少执行面信号", {
            retryable: true,
            details: {
                ability_id: input.consumerId,
                stage: "execution",
...
const applyReadinessToStatus = (status, input) => ({
    ...status,
...
export const prepareOfficialChromeRuntime = async (input) => {
    ...
    let profileState = typeof status.profileState === "string" ? status.profileState : "uninitialized";
