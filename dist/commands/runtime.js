...
const asStringArrayStrict = (value) => Array.isArray(value) && value.every((item) => typeof item === "string" && item.trim().length > 0)
    ? value.map((item) => item.trim())
    : null;
const unwrapMainWorldProbeResult = (value) => {
...
const parseSourceSampleRefs = (value) => {
...
const XHS_CLOSEOUT_VALIDATION_SOURCE_APPROVED_REASON = "XHS_CLOSEOUT_VALIDATION_SOURCE_APPROVED";
const sampleSignal = (sample) => {
    const payload = asObject(sample.structured_payload);
    return asObject(payload?.signal);
};
const sampleSourceGateAudit = (sample) => {
    const payload = asObject(sample.structured_payload);
    return asObject(payload?.source_gate_audit);
};
const isoTimeMs = (value) => {
    if (!value) {
        return null;
    }
...
const sampleMatchesSourceGateAudit = (sample, sourceAudit) => {
    ...
    return (sourceGateAudit.event_id === sourceAudit.event_id &&
...
const sourcePageUrlMatchesSourceAudit = (pageUrl, sourceAudit) => {
    if (!pageUrl) {
        return false;
    }
    try {
        const parsed = new URL(pageUrl);
        return (parsed.hostname === sourceAudit.target_domain &&
...
const signalMatchesSourceBinding = (input) => {
    ...
    const browserBindingMatches = browserEvidence?.target_domain === input.sourceAudit.target_domain &&
    ...
    return (asString(rhythmProfile?.source_run_id) === input.sourceRunId &&
...
const sourceAuditMatchesXhsCloseoutScope = (input) => {
    return (input.sourceAudit.profile === input.profile &&
...
const readXhsCloseoutValidationSignalsFromSourceSamples = async (input) => {
    const samples = await Promise.all(input.sourceSampleRefs.map((sampleRef) => input.store.getAntiDetectionStructuredSample(sampleRef)));
    const artifactRefs = new Set();
    const signals = {};
    for (const [targetFrRef, validationScope] of requiredXhsCloseoutSampleScopes) {
        const sample = samples.find((candidate) => candidate !== null &&
            candidate.target_fr_ref === targetFrRef &&
            candidate.validation_scope === validationScope) ?? null;
        ...
        const sourceRhythmView = validationScope === "layer3_session_rhythm"
    ...
    if (!parsedSignals) {
        throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source samples did not cover all required scopes", {
            retryable: false,
            details: {
                ability_id: "runtime.xhs_closeout_validation",
                stage: "execution",
    ...
    return {
        ...
        sourceSamples: samples.filter((sample) => sample !== null)
    };
};
const assertXhsCloseoutValidationSourceAudit = async (input) => {
    const samples = await Promise.all(input.sourceSampleRefs.map((sampleRef) => input.store.getAntiDetectionStructuredSample(sampleRef)));
    const sampleGateAudits = samples.map((sample) => sample ? sampleSourceGateAudit(sample) : null);
    const sampleGateAudit = sampleGateAudits[0] ?? null;
    const allSamplesReferenceSameGateAudit = !!sampleGateAudit &&
        sampleGateAudits.every((gateAudit) => gateAudit &&
    ...
    if (allSamplesReferenceSameGateAudit &&
    ...
    const approvalRecord = typeof sourceAudit?.decision_id === "string" && sourceAudit.decision_id.length > 0
        ? await input.store.getGateApprovalByDecisionId(sourceAudit.decision_id)
        : null;
    const approvalChecks = approvalRecord?.checks ?? {};
    const approvalChecksComplete = approvalRecord?.approved === true &&
    ...
    const approvalMatches = approvalChecksComplete &&
    ...
    throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source audit is not eligible", {
        retryable: false,
        details: {
            ability_id: "runtime.xhs_closeout_validation",
            stage: "execution",
...
const assertXhsCloseoutValidationSourceUrl = (input) => {
    if (!input.href) {
        throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source page URL is missing", {
            retryable: false,
            details: {
    ...
    if (parsed.hostname !== input.expectedDomain ||
        !parsed.pathname.replace(/\/+$/u, "").endsWith("/search_result")) {
        throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source must be collected from search_result", {
            retryable: false,
            details: {
                ability_id: "runtime.xhs_closeout_validation_source",
                stage: "execution",
...
const assertXhsCloseoutValidationSourceOfficialRuntime = (input) => {
    ...
    if (input.profileMeta?.profileState !== "ready" ||
        !binding ||
        browserChannel !== "chrome") {
        throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source requires managed official Chrome runtime", {
            retryable: false,
            details: {
                ability_id: "runtime.xhs_closeout_validation_source",
                stage: "execution",
...
const assertXhsCloseoutValidationSourceOfficialBridge = (input) => {
    ...
    throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source requires official Chrome bridge transport", {
        retryable: false,
        details: {
            ability_id: "runtime.xhs_closeout_validation_source",
            stage: "execution",
...
const assertXhsCloseoutValidationSourceBrowserAttestation = (input) => {
    ...
    const attested = input.bootstrapPayload?.runtime_bootstrap_attested === true &&
    ...
    throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source requires browser attestation", {
        retryable: false,
        details: {
            ability_id: "runtime.xhs_closeout_validation_source",
            stage: "execution",
...
const isEligibleXhsCloseoutValidationSourceRhythmDecision = (rhythmView) => {
    ...
    if (isLiveAdmissionAllowedXhsCloseoutValidationSourceRhythm({
        decision,
        currentRiskState,
        nextRiskState,
        windowRiskState,
...
const isBaselineRequiredXhsCloseoutValidationSourceRhythm = (input) => input.decision === "deferred" &&
    isXhsCloseoutValidationSourceRecoverableRiskState(input.currentRiskState) &&
    isXhsCloseoutValidationSourceRecoverableRiskState(input.nextRiskState) &&
    isXhsCloseoutValidationSourceRecoverableRiskState(input.windowRiskState) &&
    XHS_CLOSEOUT_VALIDATION_SOURCE_RECOVERY_REASONS.every((reason) => input.reasonCodes.includes(reason)) &&
    input.requires.length > 0 &&
    input.requires.every((requirement) => XHS_CLOSEOUT_VALIDATION_SOURCE_BASELINE_REQUIRES.has(requirement));
const assertEligibleXhsCloseoutValidationSourceRhythm = (input) => {
    ...
    if (!sessionRhythmWindowId || !sessionRhythmDecisionId || !decisionSessionId) {
        throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source rhythm evidence is missing", {
            retryable: false,
            details: {
                ability_id: "runtime.xhs_closeout_validation_source",
                stage: "execution",
    ...
    const runAndSessionBound = decisionRunId === input.runId &&
        (input.sessionId === null ||
            input.sessionId === undefined ||
            decisionSessionId === input.sessionId);
    const allowedRhythm = decision === "allowed" &&
        currentRiskState === "allowed" &&
        nextRiskState === "allowed" &&
        windowRiskState === "allowed";
    if (!runAndSessionBound ||
        !isEligibleXhsCloseoutValidationSourceRhythmDecision(input.rhythmView)) {
        throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source rhythm evidence is not allowed", {
            retryable: false,
            details: {
                ability_id: "runtime.xhs_closeout_validation_source",
                stage: "execution",
...
const resolveXhsCloseoutValidationSourceRhythm = async (input) => {
    ...
    throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source rhythm evidence is missing for the current run", {
        retryable: false,
        details: {
            ability_id: "runtime.xhs_closeout_validation_source",
            stage: "execution",
...
const runtimeXhsCloseoutValidationSource = async (context) => {
    ...
    const actionRef = asString(context.params.action_ref) ??
    ...
    if (requestedExecutionMode !== "live_read_high_risk") {
        throw new CliError("ERR_CLI_INVALID_ARGS", "XHS closeout validation source requires live_read_high_risk mode", {
            details: {
                ability_id: "runtime.xhs_closeout_validation_source",
                stage: "input_validation",
    ...
    if (hasOwn(context.params, "signals") || hasOwn(context.params, "artifact_refs")) {
        throw new CliError("ERR_CLI_INVALID_ARGS", "XHS closeout validation source inline evidence forbidden", {
            details: {
                ability_id: "runtime.xhs_closeout_validation_source",
                stage: "input_validation",
    ...
    if (accountSafetyState !== "clear") {
        throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source blocked by account-safety state", {
            retryable: false,
            details: {
                ability_id: "runtime.xhs_closeout_validation_source",
                stage: "execution",
    ...
    if (!fingerprintContext.execution.live_allowed) {
        throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source fingerprint gate blocked live_read_high_risk", {
            retryable: false,
            details: {
                ability_id: "runtime.xhs_closeout_validation_source",
                stage: "execution",
                reason: fingerprintContext.execution.reason_codes[0] ??
    ...
    try {
        ...
        store = runtimeStore;
        const { sessionId: rhythmSessionId, sessionRhythmWindowId, sessionRhythmDecisionId, rhythmAdmissionClass, rhythmAuditRiskState, rhythmAuditNextState } = await resolveXhsCloseoutValidationSourceRhythm({
            store: runtimeStore,
            profile,
            sessionId: null,
        ...
        if (sessionId !== rhythmSessionId) {
            throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source rhythm evidence is not bound to the active session", {
                retryable: false,
                details: {
                    ability_id: "runtime.xhs_closeout_validation_source",
                    stage: "execution",
        ...
        if (!bootstrapResult.ok) {
            ...
            throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source runtime bootstrap failed", {
                retryable: false,
                details: {
                    ability_id: "runtime.xhs_closeout_validation_source",
                    stage: "execution",
                    reason: asString(bootstrapDetails?.reason) ??
        ...
        if (bootstrapStatus !== "ready" ||
            ...
            throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source runtime bootstrap ack invalid", {
                retryable: false,
                details: {
                    ability_id: "runtime.xhs_closeout_validation_source",
                    stage: "execution",
                    reason: bootstrapStatus === "stale"
        ...
        if (!probeResult.ok) {
            ...
            throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source main-world probe failed", {
                retryable: false,
                details: {
                    ability_id: "runtime.xhs_closeout_validation_source",
                    stage: "execution",
                    reason: asString(probeDetails?.reason) ??
        ...
        if (probeTargetTabId !== targetTabId) {
            throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source probe target tab mismatch", {
                retryable: false,
                details: {
                    ability_id: "runtime.xhs_closeout_validation_source",
                    stage: "execution",
        ...
        if (probe?.probe_response_received !== true) {
            throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source main-world probe did not return evidence", {
                retryable: false,
                details: {
                    ability_id: "runtime.xhs_closeout_validation_source",
                    stage: "execution",
        ...
        if (!injection ||
            ...
            throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source fingerprint evidence is not ready", {
                retryable: false,
                details: {
                    ability_id: "runtime.xhs_closeout_validation_source",
                    stage: "execution",
        ...
        await runtimeStore.runInTransaction(async () => {
            ...
            sourceAuditRef.current = await runtimeStore.appendGateAuditRecord({
                ...
                gateReasons: rhythmAdmissionClass === "baseline_required_recovery"
                    ? [
        ...
        if (!sourceAudit) {
            throw new CliError("ERR_EXECUTION_FAILED", "XHS closeout validation source audit was not persisted", {
                retryable: false,
                details: {
                    ability_id: "runtime.xhs_closeout_validation_source",
                    stage: "execution",
...
const runtimeXhsCloseoutValidation = async (context) => {
    const targetDomain = asString(context.params.target_domain);
    const requestedExecutionMode = asString(context.params.requested_execution_mode);
    ...
    if (requestedExecutionMode !== "live_read_high_risk") {
        throw new CliError("ERR_CLI_INVALID_ARGS", "XHS closeout validation requires live_read_high_risk mode", {
            details: {
                ability_id: "runtime.xhs_closeout_validation",
                stage: "input_validation",
...
const assertRuntimeRestoreXhsTargetSafetyGate = async (context) => {
    ...
    const officialRuntimeReady = status.identityBindingState === "bound" &&
        ...
        status.executionSurface === "real_browser" &&
        status.headless === false;
    const attachedRuntimeReady = attachedRuntimeForRestore &&
    ...
    const managedRuntimeTargetPage = asString(runtimeTakeover?.managedTargetPage ?? runtimeTakeover?.managed_target_page);
    const runtimeTakeoverMode = asString(runtimeTakeover?.mode);
    const staleBootstrapRecoveryContinuity = runtimeTakeoverMode === "stale_bootstrap_rebind" &&
    ...
    const staleBootstrapSameRuntimeReady = restoreRequestedAt !== null &&
        status.lockHeld === true &&
...
const runtimeRestoreXhsTarget = async (context) => {
    let bridge = null;
    try {
        ...
        if (!result.ok) {
            ...
            const structuredReason = typeof details?.reason === "string" && details.reason.trim().length > 0
                ? details.reason.trim()
                : result.error.code;
            const semanticRestoreDenial = semanticRestoreDenialReasons.has(structuredReason);
            throw new CliError(semanticRestoreDenial ? "ERR_EXECUTION_FAILED" : "ERR_RUNTIME_UNAVAILABLE", result.error.message, {
                retryable: !semanticRestoreDenial &&
