...
当 PR 落入专项门禁，或其职责属于 formal spec review PR / live evidence 治理落库 PR 时，PR 描述必须先补齐结构化 `gate_applicability`，至少包含：
...
其中，`review_lane=governance_landing_pr` 只有在 reviewer / guardian 同时确认“PR 元数据显式引用 `#310`”且“实际变更精确等于以下五处冻结治理落库目标文件”时才成立：
...
若带 `#310` 上下文的 PR 只命中上述目标文件子集，或在五处目标文件之外扩 scope，必须直接保持 blocked，不得降格成 `general_pr`。

若 PR 实际变更精确命中上述五处治理落库目标文件，但 PR 元数据没有显式引用 `#310`，也必须直接保持 blocked，不得退回普通 PR。
...
- formal spec review PR 与治理落库 PR 的 split 是强约束；一旦 spec 正式契约或 `TODO.md` handoff 回写与治理落库文件同 PR 出现，必须先拆分再继续
