...
以下门禁只适用于声称完成真实 runtime、真实页面交互、真实 live read/write 闭环，或以 live evidence 作为关闭依据的 PR：

- 需要 latest head 的重新复验；旧 head、旧 run、旧 artifact 不能直接沿用
...
进入 `merge-ready` 前，此类 PR 必须在 PR 描述中补齐 live evidence 专区，至少包含：
...
若上述证据缺失、失效或边界不符，该 PR 只能保持 `Refs #...` 与非 `merge-ready` 状态，不得按真实闭环完成申报。
