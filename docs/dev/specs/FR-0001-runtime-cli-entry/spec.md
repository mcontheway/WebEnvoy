...
3. 定义统一的 `stdout` 成功/错误响应、`stderr` 日志边界、稳定退出码和 `run_id` 语义。
4. 为 `#142`、`#143`、`#145`、`#154` 提供不需要推翻的 CLI 承载面。
5. 只冻结最小稳定契约，不提前承诺完整命令面、SDK、API 或 daemon 形态。
...
5. 本 FR 没有提前承诺 SDK / API、完整命令面、L2 原语或平台读写实现。
6. 至少存在与本 FR 风险相匹配的最小验证方向：
   - CLI 入口可调用
   - 已实现命令成功输出
   - 未知命令输出
   - 未实现命令输出
