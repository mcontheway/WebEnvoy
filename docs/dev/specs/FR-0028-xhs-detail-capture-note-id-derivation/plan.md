...
- 规约对照：
  ...
  - 对照 `extension/xhs-read-execution.ts` 与 `tests/xhs-read-execution.fallback.test.ts`，确认 response-side admitted evidence 只认可 detail note candidate record 上的 `note_id` / `noteId` / `id`，metadata-only note id 不构成 success evidence
- 文档门禁：
  - `git diff --check`
  - `git status --short`
- 纯度校验：
