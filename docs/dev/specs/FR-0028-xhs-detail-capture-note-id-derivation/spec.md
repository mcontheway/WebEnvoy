...
- current implementation 已经存在一条更窄、且可被 in-tree tests 直接支撑的 response-side truth：detail 请求返回成功时，admitted canonical `note_id` 至少可以从 `body.data.note` 或 `body.data.items[*].note_card` 这两类 response candidate record 上的 `note_id` / `noteId` / `id` 导出。metadata-only note id 不构成 success evidence；除这两类已验证路径外，current matcher 的其他分支仍只属于实现观察，不在本 FR 冻结为 formal truth。
...
系统必须冻结：`xhs.detail` admitted template 的 capture-side canonical `note_id` derivation 在 current v1 有且只有一条 formal owner，即基于 response-side detail response candidate record 的 canonical derivation。
...
- current v1 admitted response candidate record 的 matcher 边界必须冻结到以下 response-root / entry family：
  - response root 当前只冻结 `body.data`
  - admitted candidate entry 当前只冻结 `.note` 与 `.items[*].note_card`
- 因此，当前已被 tests 直接接纳的 `body.data.note` 与 wrapped detail payload `body.data.items[*].note_card`，属于 admitted response candidate record 的 formal scope。
- 除上述两类已验证 admitted path 外，current matcher 仍可能遍历的 bare-body root、self root、其他 direct entry 与递归路径，当前一律不在本 FR 冻结；这些路径既不能被本 FR 宣告为 admitted truth，也不能被倒推出为长期稳定契约。
...
- current v1 formal 只把 `body.data.note` 与 `body.data.items[*].note_card` 这两类 detail response candidate record 冻结为 admitted scope。
- 因此，当前唯一被 formal 接纳的 wrapped note-id-bearing field 是 `body.data.items[*].note_card` 上的 `note_id` / `noteId` / `id`；除此之外的 wrapper / record 即使当前实现仍会观察，也都不在本 FR 冻结。
...
And response 只在 metadata 或 current matcher 未接受的 wrapper 字段中出现 note-id-like 值
And response 中不存在命中目标的 detail response candidate record
When 系统执行 current v1 capture-side canonical `note_id` derivation
...
And 当前 response-side 仍缺少 admitted response candidate record
...
### 场景 5：response-side note fields 只在已验证 admitted candidate record 上成立

Given response payload 的 `body.data.note` 中存在携带 `id` 的 detail response candidate record
And 同一 response 的 metadata 中也存在 `current_note_id`
When 系统判断 admitted canonical derivation source
Then admitted truth 只能来自该 `body.data.note` record 上的 `id`
...
- response payload 缺少 `body.data.note` 或 `body.data.items[*].note_card` 这两类已验证 admitted candidate record 时，detail admitted canonical derivation 失败；这不等于 identity 变化，只代表当前 artifact 不可进入 admitted template。
- response payload 虽然包含其他 wrapper-shaped object、bare-body root 或 current implementation 仍会遍历的 nested path，但只要它们不属于 `body.data.note` 或 `body.data.items[*].note_card`，当前都不能被本 FR 提升为 admitted matcher scope。
...
2. admitted matcher boundary 已冻结为 `body.data.note` 与 `body.data.items[*].note_card` 这两类已验证 response candidate path。
