...
- 当前 tests 已直接覆盖 `body.data.note` 成功、`body.data.items[*].note_card` 成功，以及 `body.data.items[*]` target-missing 检查；其余分支主要由 current main 实现代码提供 observable truth，仍属于本契约冻结范围。
