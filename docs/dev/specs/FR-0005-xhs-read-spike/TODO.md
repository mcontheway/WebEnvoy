...
- [ ] 为 `detail/user_home` 提供或维持 repo 内可复核的 managed-profile official-runtime 正式命令面与 replay 路径；`runtime.tabs` 等 internal bridge diagnostics 不构成替代；在该 formal command surface 未稳定前，不判定对应场景的 live closeout
