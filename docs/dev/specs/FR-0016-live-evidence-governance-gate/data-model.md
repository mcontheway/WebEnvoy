...
- 约束：
  ...
  - 若 PR 自报 `review_lane=governance_landing_pr` 却缺少 `governance_issue_ref`，必须产出 `missing_governance_issue_ref` 并阻断。
  - 若后续治理维护 PR 仅因命中 `governance_scope_targets`，但不承接 `#310` 的一次性落库闭环，不得被自动吸入 `governance_landing_pr`。
...
- 约束：
  ...
  - 若 PR 自报 `review_lane=governance_landing_pr` 却缺少 `#310` 引用，不得退回 `general_pr`。
...
- 约束：
  ...
  - 若 PR 自报 `review_lane=governance_landing_pr` 却缺少 `#310` 引用，必须包含 `missing_governance_issue_ref`，且 `status=blocked`。
