...
- 集成约束：`#208/#209` 在恢复后续 live 前需共享统一风险状态机与阻断策略。
...
| U5 | `#208/#209` 可共享同一状态机并保持差异化阻断 | `FR-0009` 规约基线 + FR-0011 contract 草案 | 契约可判定性审查 | M2 | 86% | 通过 issue_action_matrix 固化 |

## Gap 清单（从 FR-0009 到 FR-0011）

| 维度 | FR-0009 已冻结 | FR-0011 需补齐 |
|---|---|---|
...
| 统一集成 | `#208/#209` 需共同受控 | 共享三态差异化阻断矩阵与审计字段 |
