...
8. `FR-0020` registry 的 shared upstream scope 不包含 `platform`、`target_domain` 或 `goal_kind`；这些键继续属于 `FR-0022` 的 downstream writable isolation，同一条上游 `active_baseline_ref` 不得被多个 `(platform, target_domain, goal_kind)` Layer 4 状态对象复用，若发生则视为隔离破坏。
