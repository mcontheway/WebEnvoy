...
- `action_mix` 至少覆盖 `navigate`、`locate`、`extract`、`reveal_only_click`、`type`、`submit` 的计数或比率。

## 2. `platform_behavior_baseline_state`

用途：

- 记录 `(profile, platform)` 维度的长期行为基线状态。
...
- `(profile, platform)` 是可写隔离主键，不允许跨 profile 或跨 platform 共用同一可写状态对象。
