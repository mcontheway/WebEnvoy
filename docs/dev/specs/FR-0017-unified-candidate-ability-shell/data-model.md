...
- `*_contract_ref` 的 authoritative resolver 是该 `ability_id` 对应的 descriptor-owned contract registry；实现层只能按完整 ref 精确 lookup，不得把 ref 退化为路径推断、runtime-store 主键或私有缓存键。
