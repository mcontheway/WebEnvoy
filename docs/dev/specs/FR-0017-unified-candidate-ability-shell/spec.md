...
4. 为 L2 单独发明第二套能力描述对象：视为范围漂移。
5. 把 `candidate_status` 直接等同于“验证通过”或“可交付”：视为与 `#155` / Phase 3 边界冲突。
6. 将下载能力排除在统一模型之外：视为与 roadmap 的统一表达方向冲突。
7. 在本 FR 中引入版本治理、导入安装或分享协议：视为越界到 Phase 3/5。
