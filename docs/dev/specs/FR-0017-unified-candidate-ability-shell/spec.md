...
- `*_contract_ref` 的 authoritative resolver 必须是 descriptor-owned contract registry；lookup 规则固定为“以当前 `ability_id` 为 owner，按完整 ref 精确匹配并校验契约种类”。实现不得把 ref 解释为 repo 文件路径、runtime-store 行键或其他私有定位规则。
...
3. `*_contract_ref` 不走 descriptor-owned contract registry，而由实现自行猜 repo 路径或重建私有映射：视为契约解析边界未冻结。
4. `ability.action` 与 `candidate_ability_descriptor.ability_kind` 不一致时仍允许继续执行：视为调用边界未冻结。
5. 为 L2 单独发明第二套能力描述对象：视为范围漂移。
6. 把 `candidate_status` 直接等同于“验证通过”或“可交付”：视为与 `#155` / Phase 3 边界冲突。
7. 将下载能力排除在统一模型之外：视为与 roadmap 的统一表达方向冲突。
8. 在本 FR 中引入版本治理、导入安装或分享协议：视为越界到 Phase 3/5。
