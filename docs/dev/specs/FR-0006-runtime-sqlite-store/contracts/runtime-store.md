...
- store 层返回的 `run` / `events` 只表达历史证据，不可直接替代 `#356` 的实时会话状态，也不可替代 `#360` 的能力真相源。
- `session_id`、`profile_name` 在 store 中属于引用字段；消费者不得据此反向修改 `#356` 状态。
- `#360` 如需形成能力结果或错误壳，必须先取自身真相数据，再按 `run_id` 读取 store 证据；不得把 store 响应直接冒充能力壳。
