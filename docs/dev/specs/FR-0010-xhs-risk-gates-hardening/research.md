...
- Primary unknowns：
  ...
  - U5：`#218/#219/#221` 并行推进时，如何避免门禁字段对象漂移。

## 当前基线

- `FR-0009` 已冻结门禁规约方向，但尚未进入实现。
- `#208` 仍未完成，`#209` 已落地但后续 live 扩展未受硬门禁约束。
- `#218/#219/#221` 为 Sprint 2 的实现分解，但当前缺统一冻结字段对象。
...
| U5 | `#218/#219/#221` 并行推进可能导致字段口径漂移 | `#218/#219/#221` issue 需求差异 + FR-0010 现有文本 | 契约差异审查 | M2 | 86% | 需在 FR-0010 冻结统一消费字段 |
...
- Rationale：
  ...
  - `#218/#219/#221` 已收敛到同一消费字段对象，降低并行实施时的契约漂移风险。
