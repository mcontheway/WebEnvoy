...
- `#208` 仍是当前需要消费 FR-0009 门禁前置的 active gate consumer；其后续真实 live 恢复 / replay / 扩展边界已由下游 formal 套件提出并进入 review，但这不自动等于可以直接放行。
...
- 证据：
  - `/Users/mc/dev/WebEnvoy/src/commands/xhs.ts`（`xhs.search` 命令入口）
  - `/Users/mc/dev/WebEnvoy/extension/content-script-handler.ts`（主世界签名调用与页面内 `fetch`）
  - `/Users/mc/dev/WebEnvoy/extension/xhs-search.ts`（`/api/sns/web/v1/search/notes` 请求组装、trace 标识、执行失败分类）
- 为什么是风险：
...
- 证据：
  - `/Users/mc/dev/WebEnvoy/extension/background.ts`（`#resolveTargetTabId` 对 `xhs.search` 的 URL 模式匹配与自动选页）
  - `/Users/mc/dev/WebEnvoy/tests/extension.service-worker.contract.test.ts`（将自动选页行为固化为契约）
- 为什么是风险：
...
- 证据：
  - `/Users/mc/dev/WebEnvoy/extension/content-script-handler.ts`（`mainWorldCall`、`CustomEvent` 回传）
- 为什么是风险：
...
- 证据：
  - `/Users/mc/dev/WebEnvoy/docs/dev/architecture/system-design/read-write.md`（富文本合成事件链、上传注入路径与局限）
  - `/Users/mc/dev/WebEnvoy/docs/dev/specs/FR-0008-xhs-write-spike/spec.md`
  - `/Users/mc/dev/WebEnvoy/docs/dev/specs/FR-0008-xhs-write-spike/risks.md`
- 为什么是风险：
...
- 证据：
  - `/Users/mc/dev/WebEnvoy/docs/dev/architecture/anti-detection.md`（主流风控重心、覆盖现状）
- 为什么是风险：
...
| 写路径交互 | 真实点击优先、必要时合成事件回退、上传候选路径分级 | `#208` 继续作为当前 gate consumer，其后续正式验证边界已在下游 formal 套件中提出，但仍以其 review 结论为准 | 写路径仍停在 Spike/候选层（`FR-0008`）及后续 replay / 恢复语义冻结阶段 | 在证据不足时继续 live 写实验，风险放大且难归因 |
...
| U2 | `#208` 后续 live / replay / 恢复事项应先被门禁约束 | `#208` + `#213` | 依赖关系审查 | M2 | 90% | 需写入正式前置条件 |
