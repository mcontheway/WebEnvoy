...
- `#208` 的后续真实 live 恢复 / replay / 扩展边界已由下游 formal 套件提出并进入 review，但这不自动等于可以直接放行。
...
| 写路径交互 | 真实点击优先、必要时合成事件回退、上传候选路径分级 | `#208` 的后续正式验证边界已在下游 formal 套件中提出，但仍以其 review 结论为准 | 写路径仍停在 Spike/候选层（`FR-0008`）及后续 replay / 恢复语义冻结阶段 | 在证据不足时继续 live 写实验，风险放大且难归因 |
