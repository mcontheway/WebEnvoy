...
| 写路径交互 | 真实点击优先、必要时合成事件回退、上传候选路径分级 | 尚无稳定写路径能力在 Phase 1 落地；`#208` 目前仅完成 issue 线程与下游 formal 套件中的正式验证边界提案 | 写路径仍停在 Spike/候选层（`FR-0008`）及后续 replay / 恢复语义冻结阶段 | 在证据不足时继续 live 写实验，风险放大且难归因 |
