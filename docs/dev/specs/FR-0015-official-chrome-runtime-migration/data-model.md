...
- `attachableReadyRuntime=true` 只表示“对一个新的 `run_id` 来说，当前 ready runtime 已被证明可安全接管并重新 bootstrap”
...
- `attachableReadyRuntime` 只表达 handoff 事实本身；真正执行 attach/rebind 前，当前调用方仍必须先持有 FR-0003 profile 独占锁
- `orphanRecoverable=true` 只表示“对一个新的 `run_id` 来说，当前 recoverable runtime 已被证明仍可安全 attach/rebind”
...
- `orphanRecoverable` 只适用于尚未有 controller 重新取得有效独占控制的 pre-lock handoff 视图；一旦 replacement controller 或其他 controller 重新持有有效独占锁，该字段必须回落为 `false`
- `orphanRecoverable` 同样只表达 handoff 事实本身；真正执行 attach/rebind 前，当前调用方仍必须先持有 FR-0003 profile 独占锁
