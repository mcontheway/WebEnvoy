...
本文档用于把 FR-0015 当前 canonical issue 页面收口到 implementation-ready 输入，不修改 `spec.md` 的正式语义，也不混入运行时代码。历史 `#281/#361` 只保留为实现链路参考，不再承担当前 formal 主树职责。

本文档冻结的是：

- FR-0015 当前 implementation-prep 应先承接哪些真实代码路径
- 哪些边界继续沿用 `#279/PR #283`、FR-0002、FR-0003
- 后续实现第一刀应怎么切，哪些范围必须继续排除
- 最小验证矩阵、stop-ship 条件与回滚边界

本文档不是功能实现承诺，也不代表 FR-0015 已实现闭环。

当前 formal 对应关系以 GitHub 上的 FR-0015 canonical issue 页面为准。已关闭的 `#281` 与 `#361` 只保留为历史实现链路参考；本文档继续负责冻结实现输入。下游实现 issue / PR 可以显式追溯 `#281/#361`，但不得把它们写成对当前 canonical issue 的替代，也不得把历史 issue 升格为 formal 契约来源。
...
- 当前 formal 对应关系以 GitHub 上的 FR-0015 canonical issue 页面为准；新的 implementation-prep 或收口事项必须挂到 FR-0015 主树下。
- `#281` 与 `#361` 只保留为历史实现链路参考；scope、stop-ship、验证入口与恢复边界仍只以本文件、`spec.md`、`plan.md`、`contracts/`、`risks.md` 为准，不要求 issue 正文承担 formal 契约职责。
- 后续若仍需补 implementation-prep / backlog handoff 后续事项，应在不外扩 scope 的前提下至少显式 `Refs` FR-0015 当前 canonical issue。纯验证后续事项继续归属 `FR-0020`，不在 FR-0015 主树下重复挂接。
