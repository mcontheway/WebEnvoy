...
- [ ] 第一刀切片必须限定在 `#281` 约束内：仅包含 identity preflight、`runtime_bootstrap_envelope` contract、`runtime.status` read model、bootstrap 失败后的 stop/retry/recover 边界，不外扩到安装器产品化、candidate 分发或 `#239` 验证体系。
