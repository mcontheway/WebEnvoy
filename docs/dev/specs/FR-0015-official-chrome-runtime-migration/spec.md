...
3. persistent identity 与 run/session bootstrap 的分层边界已可直接指导实现。
4. 后续实现 PR 的第一刀切片、状态语义与失败面已明确，不再依赖口头补充。
5. 文档已明确哪些内容仍然不属于 `#281`。
6. 本 FR 未把 candidate 安装路径、最终安装器设计或 `#239` 验证体系误写入当前正式范围。
