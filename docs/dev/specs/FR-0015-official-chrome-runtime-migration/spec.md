...
- 本 FR 必须为后续实现 PR 指定推荐的第一步切片、共享契约与状态边界。
...
- `RuntimeTakeoverEvidence.staleBootstrapRecoverable=true` 只允许表示“旧 run 的 bootstrap ack 已 stale，但同一 managed runtime 与同一 target tab/domain/page 仍可由新 run 接管后重新 bootstrap”的受限 handoff fact；不得替代 ready ack 或业务命令放行门禁。
- 上述 pre-lock handoff facts 都是 additive 派生事实；不得借它们重解释 `lockHeld`、`transportState`、`bootstrapState`、`runtimeReadiness` 的既有单实例语义。
- FR-0015 必须新增 formal-only 的 `RuntimeTakeoverEvidence` 概念，用于表示锁切换前冻结下来的 handoff 证据；它至少需要区分 `ready_attach`、`recoverable_rebind` 与 `stale_bootstrap_rebind` 三类模式，并包含 freshness、identity bound、owner conflict free、controller/browser continuity、transport/bootstrap viability、`observedRunId` 与 `runtimeContextId` 这些最小判定事实；`requestRunId`、`requestRuntimeContextId` 与 `observedRuntimeInstanceId` 只属于 `mode=stale_bootstrap_rebind` 的 non-null proof fields。
...
### 场景 3C：stale bootstrap handoff 只能用于同 target 重新 bootstrap

Given 一个新的 `run_id` 正在查询现存 official Chrome runtime
And 当前调用方自己尚未持有该 profile 的独占锁
And 当前 bootstrap request 已收到 `status=stale` ack
And 当前 `runtime.bootstrap.params` 携带 `requested_at`、`target_domain`、`target_tab_id`、`target_page` 机器字段
And `runtime.bootstrap.ack.result.stale_provenance` 非空，字段仅使用 `kind`、`observed_run_id`、`observed_runtime_context_id`、`observed_runtime_instance_id`
And `stale_provenance.kind` 为 `ready_marker | observed_runtime`
And `stale_provenance.observed_run_id` 与 `stale_provenance.observed_runtime_context_id` 均为非空字符串，且分别不等于当前 `(run_id, runtime_context_id)`
And `stale_provenance.observed_runtime_instance_id` 为非空字符串
And stale-bootstrap recovery proof fields 均来自 `runtime-readiness-status` contract 冻结的 source machine fields，包括 target proof 与 freshness proof
When `runtime.status` 评估 stale-bootstrap handoff 条件
Then `runtime.status` 顶层仍只允许返回 `runtimeReadiness=blocked` 与 `bootstrapState=stale`
And 只有在 `identityBindingState=bound`、`transportState=ready`、`stale_provenance` 完整、同一 managed target tab/domain/page 已由 `managedTargetTabId` / `managedTargetDomain` / `managedTargetPage` 证明、official Chrome `real_browser`、`headless=false`、owner conflict free、controller/browser continuity、freshness 与消费方既有 safety/validation gates 均成立时，才允许 `RuntimeTakeoverEvidence.staleBootstrapRecoverable=true`
And 冻结出的 `RuntimeTakeoverEvidence.requestRunId` 与 `requestRuntimeContextId` 必须分别等于当前 bootstrap request 的 `(run_id, runtime_context_id)`
And 冻结出的 `RuntimeTakeoverEvidence.observedRuntimeInstanceId` 必须等于 `stale_provenance.observed_runtime_instance_id`
And `RuntimeTakeoverEvidence.staleBootstrapRecoverable=true` 不得被解释为业务命令已可直接执行
And replacement `run_id` 必须先 attach/rebind 当前 run，再重新下发当前 `(run_id, runtime_context_id)` 的 `runtime.bootstrap`
And 旧 run 的 stale ack 不得被直接接受为 replacement `run_id` 的 ready ack

### 场景 3D：post-lock attach/rebind 只能消费锁切换前冻结的 evidence

Given 一个新的 `run_id` 已在锁切换前观察到 `RuntimeTakeoverEvidence.attachableReadyRuntime=true`、`RuntimeTakeoverEvidence.orphanRecoverable=true` 或 `RuntimeTakeoverEvidence.staleBootstrapRecoverable=true`
...
And 若 evidence 为 `mode=stale_bootstrap_rebind`，`postLockTakeoverGate` 还必须复验冻结的 `managedTargetTabId`、`managedTargetDomain`、`managedTargetPage` 与当前 post-lock observed target 完全一致
And 若 evidence 为 `mode=stale_bootstrap_rebind`，`targetTabContinuity` 必须仍为 `runtime_trust_state`
And 若 evidence 为 `mode=stale_bootstrap_rebind`，`postLockTakeoverGate` 必须复验 `requestRunId` 与 `requestRuntimeContextId` 等于当前 replacement run 的 `(run_id, runtime_context_id)`
...
5. 后续实现 PR 的第一步切片、状态语义与失败面已明确，不再依赖口头补充。
