...
- successor detail implementation path 还必须继续等待 `#510` 这条 required detail-path gate；该 gate 只约束 detail path，不替代 `#508` 对 shared request-context 语义的 ownership，本契约不在此重述其 owning suite scope。
