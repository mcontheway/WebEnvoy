...
- [x] 实现 `fingerprint_profile_bundle` 的生成/加载/校验。
- [x] 实现 `fingerprint_patch_manifest` 的构造与补丁前置校验。
- [x] 落地 P0 补丁：`AudioContext`、Battery API、`navigator.plugins`、`navigator.mimeTypes`。
- [x] 落地 profile 环境绑定校验与失配处理。
- [ ] 视切片安排继续落地 P1/P2 补丁。
- [x] 为 profile 元数据、manifest、环境绑定与错误分类补齐自动化测试。
- [ ] 在检测站点上补齐集成验证证据。
