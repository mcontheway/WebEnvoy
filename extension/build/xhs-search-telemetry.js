...
import { buildUnifiedRiskStateOutput, resolveRiskState as resolveSharedRiskState } from "../shared/risk-state.js";
...
export const resolveXsCommon = (value) => {
    if (typeof value === "string" && value.trim().length > 0) {
        return value.trim();
    }
    return "{}";
};
export const containsCookie = (cookie, key) => cookie
    .split(";")
    .map((item) => item.trim())
