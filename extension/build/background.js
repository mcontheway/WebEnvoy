...
const isRestoreSafetyGateManagedTabContinuityAllowed = (value, profile, runId, targetDomain, targetTabId) => {
    const runtimeContextId = asNonEmptyString(value?.runtime_context_id);
    const continuity = asRecord(value?.managed_tab_continuity);
    return (runtimeContextId !== null &&
...
class ChromeBackgroundBridge {
    ...
    async #handleRuntimeRestoreXhsTarget(request) {
        ...
        try {
            ...
            fail(this.chromeApi.tabs.get ? "TARGET_RESTORE_TARGET_TAB_NOT_FOUND" : "TARGET_RESTORE_TAB_QUERY_FAILED", error instanceof Error ? error.message : String(error), this.chromeApi.tabs.get ? { requested_target_tab_id: targetTabId } : undefined);
            return;
        }
        if (!explicitTarget) {
        ...
        const safetyGateBindsAttachedRuntimeTarget = isRestoreSafetyGateManagedTabContinuityAllowed(restoreSafetyGate, profile, runId, targetDomain, sourceTab.id);
        ...
        if (xhsRestoreSearchUrlsMatch(previousUrl, targetUrl)) {
            ...
            try {
                ...
                fail("TARGET_RESTORE_NAVIGATION_FAILED", error instanceof Error ? error.message : String(error), { previous_url: previousUrl, target_url: targetUrl });
                return;
            }
        }
