...
const resolveFingerprintContext = (
  ...
  const direct =
    resolveAttestedFingerprintRuntimeContext(commandParams.fingerprint_context) ??
...
const normalizeIssue209AdmissionDraft = (
...
const isRestoreSafetyGateManagedTabContinuityAllowed = (
  value: Record<string, unknown> | null,
  profile: string,
  runId: string,
  targetDomain: string | null,
  ...
  const runtimeContextId = asNonEmptyString(value?.runtime_context_id);
  const continuity = asRecord(value?.managed_tab_continuity);
  return (
    runtimeContextId !== null &&
    continuity?.source === "cli_runtime_attach" &&
    continuity.profile_ref === profile &&
    continuity.run_id === runId &&
    continuity.runtime_context_id === runtimeContextId &&
    continuity.target_domain === targetDomain &&
    continuity.target_tab_id === targetTabId
  );
};
...
const isCreatorArticlePublishPage = (url: string, domain: string): boolean => {
...
const applyCanonicalXhsForwardCommandParams = (input: {
  ...
  approvalRecord?: XhsApprovalRecord | null;
  admissionContext?: Record<string, unknown> | null;
}): Record<string, unknown> => {
  const normalized: Record<string, unknown> = { ...input.commandParams };
...
const buildCanonicalGateAuditArtifacts = (input: {
  ...
  targetTabId: number | null;
  targetPage: string | null;
  actualTargetDomain: string | null;
  actualTargetTabId: number | null;
  actualTargetPage: string | null;
...
const buildGateOnlyObservability = (
  gatePayload: Record<string, unknown>
): Record<string, unknown> => {
    ...
    failure_site:
      gateOutcome.gate_decision === "blocked"
        ? {
            ...
            component: "gate",
            target:
              asNonEmptyString(gateInput.target_page) ??
              asNonEmptyString(gateInput.target_domain) ??
              "issue_208_gate_only",
...
const createBackgroundXhsGatePayload = (input: {
  ...
  issueScope: IssueScope;
  riskState: XhsRiskState;
  resolvedRiskState: XhsRiskState;
  targetDomain: string | null;
  targetTabId: number | null;
  targetPage: string | null;
  actionType: XhsActionType | null;
  ...
  gateReasons: string[];
  requiresManualConfirmation: boolean;
  fingerprintGateDecision: "allowed" | "blocked";
...
export class BackgroundRelay extends ExtractedBackgroundRelay {
  constructor(
    contentScript: ContentScriptHandler,
...
class ChromeBackgroundBridge {
  ...
  #rememberTrustedFingerprintContext(
    ...
    if (!canPrimeFromBootstrap) {
      ...
      this.#promoteRuntimeBootstrapStateFromExecutionSignal(
        profile,
        sessionId,
        fingerprintRuntime,
    ...
    if (!explicitSessionId || explicitSessionId !== this.#sessionId) {
      ...
      if (hasInstalledFingerprintInjection(fingerprintRuntime)) {
        this.#promoteRuntimeBootstrapStateFromExecutionSignal(
          ...
          explicitSessionId,
          fingerprintRuntime,
          asNonEmptyString(startupTrust.run_id ?? null),
          asNonEmptyString(startupTrust.runtime_context_id ?? null),
          senderBinding
        );
      }
      if (!senderBinding) {
  ...
  async #waitForRestoredTargetNavigation(input: {
    ...
    | {
        ok: false;
        reason: "TARGET_RESTORE_TARGET_TAB_NOT_FOUND" | "TARGET_RESTORE_NAVIGATION_NOT_READY";
        message: string;
  ...
  async #handleRuntimeRestoreXhsTarget(request: BridgeRequest): Promise<void> {
    ...
    try {
      ...
      fail(
        this.chromeApi.tabs.get ? "TARGET_RESTORE_TARGET_TAB_NOT_FOUND" : "TARGET_RESTORE_TAB_QUERY_FAILED",
        error instanceof Error ? error.message : String(error),
        this.chromeApi.tabs.get ? { requested_target_tab_id: targetTabId } : undefined
      );
      return;
    }
    ...
    const bootstrap = this.#runtimeTrustState.getBootstrap(profile);
    const trusted = this.#runtimeTrustState.getTrusted(profile, sessionId);
    const restoreRuntimeContextId = asNonEmptyString(restoreSafetyGate?.runtime_context_id);
    const bootstrapBindsTarget =
      !!bootstrap &&
      (bootstrap.status === "pending" || bootstrap.status === "ready") &&
      bootstrap.sessionId === sessionId &&
      bootstrap.runtimeContextId === restoreRuntimeContextId &&
    ...
    const safetyGateBindsAttachedRuntimeTarget = isRestoreSafetyGateManagedTabContinuityAllowed(
      restoreSafetyGate,
      profile,
      runId,
      targetDomain,
    ...
    if (!bootstrapBindsTarget && !trustedBindsTarget && !safetyGateBindsAttachedRuntimeTarget) {
      ...
      const results = await this.chromeApi.scripting.executeScript({
        ...
        func: async (
          ...
          const namespaceEvent =
            typeof namespaceEventName === "string" ? namespaceEventName : "";
          const state = {
    ...
    if (XHS_GATE_COMMANDS.has(command)) {
      ...
      if (!gateResult.allowed || (!gateResult.targetTabId && !gateResult.gateOnly)) {
        const gateFailureReason =
          gateResult.consumerGateResult.gate_reasons[0] ?? "TARGET_TAB_NOT_EXPLICIT";
  ...
  async #evaluateXhsTargetGate(request: BridgeRequest): Promise<XhsTargetGateResult> {
    ...
    const {
      commandParams,
      targetDomain,
      targetTabId: initialTargetTabId,
      targetPage,
      issueScope,
      riskState,
      actionType,
    ...
    let actualTargetDomain: string | null = null;
    let actualTargetTabId: number | null = null;
    let actualTargetPage: string | null = null;
    let actualTargetUrl: string | null = null;
    ...
    const resolvedTargetDomainForLookup =
      targetDomain !== null && XHS_DOMAIN_ALLOWLIST.has(targetDomain) ? targetDomain : null;
    const actualTargetQueryPattern =
      resolvedTargetDomainForLookup !== null
        ? buildChromeUrlPatternForDomain(resolvedTargetDomainForLookup)
        : null;
    const shouldResolveActualTargetContext =
      actualTargetQueryPattern !== null &&
      targetTabId !== null &&
      targetPage &&
    ...
    const canonicalApprovalPayloadRecord: XhsApprovalRecord =
      canAdoptCanonicalLiveAdmission && sharedCanonicalApprovalRecord
        ? {
  ...
  async #ensureMainWorldBridgeInjectedInternal(request: BridgeRequest, tabId: number): Promise<void> {
    ...
    if (
      !forceReinjectForStagedExtension &&
      probeSecret &&
  ...
  #shouldForceStagedMainWorldBridgeReinject(): boolean {
    const manifest = this.chromeApi.runtime.getManifest?.();
    const contentScripts = Array.isArray(manifest?.content_scripts) ? manifest.content_scripts : [];
    return contentScripts.some((entry) =>
      Array.isArray(entry?.js) && entry.js.includes(STAGED_EXTENSION_BOOTSTRAP_SCRIPT_PATH)
    );
  }
