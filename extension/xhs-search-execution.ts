...
type RequestContextLookupResult =
  | {
      state: "hit";
...
const deriveSearchShapeFromSource = (value: unknown): SearchRequestShape | null => {
  const record = asRecord(value);
  if (!record) {
    return null;
  }
...
const deriveSearchShapeFromCommand = (params: XhsSearchParams): SearchRequestShape => ({
  command: "xhs.search",
  method: "POST",
  pathname: SEARCH_ENDPOINT,
...
const deriveSearchShapeFromArtifact = (value: unknown): SearchRequestShape | null => {
  const record = asRecord(value);
  if (!record) {
    return null;
  }
...
const serializeSearchShape = (shape: SearchRequestShape): string =>
  JSON.stringify({
...
const resolveCapturedArtifactSearchId = (value: unknown): string | null => {
  const record = asRecord(value);
  if (!record) {
    return null;
  }
...
const resolveSearchRequestContext = (
  ...
  if (
    ...
    if (admittedTemplate) {
      return resolveSearchRequestContext(
...
const failClosedForRequestContext = (
  input: {
    abilityId: string;
    expectedShape: SearchRequestShape;
    lookupResult: Exclude<RequestContextLookupResult, { state: "hit" }>;
...
export const executeXhsSearch = async (
  ...
  if (
    ...
    const validationText =
      typeof input.options.validation_text === "string" && input.options.validation_text.trim().length > 0
  ...
  const headers: Record<string, string> = {
    Accept:
      getCapturedHeader(requestContextResult.headers, "Accept") ?? "application/json, text/plain, */*",
    "Content-Type":
      getCapturedHeader(requestContextResult.headers, "Content-Type") ??
    ...
    "x-xray-traceid":
      getCapturedHeader(requestContextResult.headers, "x-xray-traceid") ??
      env.randomId().replace(/-/g, "")
  };
