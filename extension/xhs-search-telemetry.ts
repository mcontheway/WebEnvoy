...
export const classifyPageKind = (href: string): string => {
...
export type XhsAccountSafetySurfaceReason =
  | "XHS_LOGIN_REQUIRED"
  | "ACCOUNT_ABNORMAL"
  | "XHS_ACCOUNT_RISK_PAGE"
  | "CAPTCHA_REQUIRED"
...
export interface XhsAccountSafetySurface {
  reason: XhsAccountSafetySurfaceReason;
  message: string;
}

const normalizeSurfaceText = (value: string): string => value.replace(/\s+/gu, " ").toLowerCase();
...
export const classifyXhsAccountSafetySurface = (input: {
  ...
  if (
    title.includes("browser environment abnormal") &&
...
export const isTrustedEditorInputValidation = (result: EditorInputValidationResult): boolean =>
  result.ok &&
  result.mode === "controlled_editor_input_validation" &&
  result.attestation === "controlled_real_interaction";

export const resolveSimulatedResult = (
  simulated: string | undefined,
  params: { query: string; search_id?: string },
...
export const parseCount = (body: unknown): number => {
...
export const inferRequestException = (error: unknown): { reason: string; message: string; detail: string } => {
  const errorName =
...
export const resolveXsCommon = (value: unknown): string => {
  if (typeof value === "string" && value.trim().length > 0) {
    return value.trim();
  }
  return "{}";
...
export const containsCookie = (cookie: string, key: string): boolean =>
  cookie
    .split(";")
    .map((item) => item.trim())
