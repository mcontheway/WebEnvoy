...
const isEnvironmentAssignmentToken = (token: string): boolean =>
  /^[A-Za-z_][A-Za-z0-9_]*=/.test(token);

const isInspectableWrapperScriptPath = (filePath: string): boolean =>
  basename(filePath) === NATIVE_HOST_ENTRY_BASENAME ||
...
const isJavascriptWrapperScriptPath = (filePath: string): boolean =>
  INSPECTABLE_NODE_WRAPPER_SCRIPT_EXTENSIONS.has(extname(filePath).toLowerCase());

const parseLiteralShellAssignment = (
  line: string
): {
  name: string;
  value: string;
...
const substituteKnownShellVariables = (value: string, variables: Map<string, string>): string =>
  (variables.get("BASH_SOURCE[0]")
    ? value.replace(/\$\{BASH_SOURCE\[0\]\}/g, variables.get("BASH_SOURCE[0]")!)
    : value
  ).replace(/\$\{([A-Za-z_][A-Za-z0-9_]*|0)\}|\$([A-Za-z_][A-Za-z0-9_]*)|\$0/g, (match, braced, bare) => {
    const variableName =
...
const tokenizeHostCommandForInspection = (
  command: "runtime.install" | "runtime.uninstall",
  hostCommand: string
): string[] | null => {
  try {
...
const looksLikeExternalCommandToken = (token: string): boolean =>
  token.includes("/") ||
  KNOWN_WRAPPER_INTERPRETER_BASENAMES.has(basename(token)) ||
  /\.(?:sh|bash|zsh|command|js|mjs|cjs)$/i.test(token);

const splitShellStatements = (line: string): string[] => {
...
const isIgnorableScriptLine = (line: string): boolean => {
...
const extractRunnableCommandText = (
  command: "runtime.install" | "runtime.uninstall",
  line: string
): string | null => {
...
const readInspectableWrapperScript = async (filePath: string): Promise<string | null> => {
  let raw: string;
  try {
...
const isManagedNativeHostEntryPath = (entryPath: string): boolean => {
...
const isRepoOwnedNativeHostEntryPath = async (cwd: string, filePath: string): Promise<boolean> => {
...
const candidatePathReferencesRepoOwnedNativeHost = async (input: {
  ...
  if (
    KNOWN_WRAPPER_INTERPRETER_BASENAMES.has(basename(comparableCandidatePath)) &&
...
const tokenReferencesRepoOwnedNativeHost = async (input: {
  ...
  for (const rawLine of input.text.split(/\r?\n/)) {
    for (const statement of splitShellStatements(rawLine)) {
      ...
      if (
        await commandTextReferencesRepoOwnedNativeHost({
          command: input.command,
          baseDir: input.baseDir,
...
const shouldExportLegacyProfileDirForExplicitHost = async (input: {
  ...
  for (const [index, token] of tokens.entries()) {
    ...
    if (previousToken && INLINE_SHELL_COMMAND_TOKENS.has(previousToken)) {
      if (
        await tokenReferencesRepoOwnedNativeHost({
          command: input.command,
          baseDir: input.cwd,
          text: token,
          depth: 0,
...
const resolveDefaultManifestDirectory = (browserChannel: BrowserChannel): string => {
