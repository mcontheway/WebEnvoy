...
const assertNoSymlinkAncestorBetween = async (input: {
  command: "runtime.install" | "runtime.uninstall";
  field: "manifest_dir" | "launcher_path";
  fromDir: string;
  targetDir: string;
}): Promise<void> => {
...
const hashSocketPathSeed = (value: string): string => {
  let hash = 0x811c9dc5;
...
const extractLauncherProfileDir = (launcherRaw: string): string | null => {
  const match = launcherRaw.match(
    /^export WEBENVOY_NATIVE_BRIDGE_PROFILE_DIR=(?:"([^"]+)"|'([^']+)')$/m
  );
  if (!match) {
    return null;
  }
