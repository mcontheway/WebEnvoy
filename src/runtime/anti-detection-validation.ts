...
const omitNestedKey = (value: JsonObject, path: readonly string[]): void => {
...
const xhsCloseoutSignalMatchesBaseline = (input: {
  currentSignalVector: JsonObject;
  activeBaselineSignalVector: JsonObject | null;
}): boolean => {
  if (!input.activeBaselineSignalVector) {
    return true;
  }
  return (
    stableJson(canonicalizeXhsCloseoutSignalVectorForComparison(input.activeBaselineSignalVector)) ===
  ...
  for (const requiredScope of XHS_CLOSEOUT_REQUIRED_VALIDATION_SCOPES) {
    ...
    const signalMatchesBaseline = xhsCloseoutSignalMatchesBaseline({
      ...
      activeBaselineSignalVector: activeBaseline
        ? (activeBaseline.signal_vector as JsonObject)
        : null
    });
