...
import {
  ...
  type SessionRhythmStatusViewInput,
...
const toSessionRhythmIdPart = (value: string): string => value.replace(/[^A-Za-z0-9._-]+/gu, "_");
...
interface RuntimeStoreWriter {
  ...
  recordSessionRhythmStatusView?(input: SessionRhythmStatusViewInput): Promise<unknown>;
  close(): void;
}
...
const extractGateAuditRecordInput = (
  ...
  const decisionId =
    asString(auditRecord.decision_id) ??
    asString(asObject(source.gate_outcome)?.decision_id) ??
...
export class RuntimeStoreRecorder {
    ...
    if (auditInput && this.#store.appendGateAuditRecord) {
      ...
      if (approvalRequired && !approvalInput) {
        throw new RuntimeStoreError(
          ...
          "approval_record is required for allowed live audit records"
        );
      }
      ...
      if (approvalRequired && !persistedApprovalId) {
        throw new RuntimeStoreError(
          "ERR_RUNTIME_STORE_INVALID_INPUT",
          "persisted approval_id is required for allowed live audit records"
        );
      }
  ...
  async #recordSessionRhythmArtifacts(
    input: {
      profile: string | null;
