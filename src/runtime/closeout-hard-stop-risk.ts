...
export type CloseoutHardStopRiskClass =
  | "account_abnormal"
  | "captcha_required"
  | "login_security_page"
  | "browser_environment_abnormal";

export type CloseoutHardStopRiskReason =
  | "ACCOUNT_ABNORMAL"
  | "CAPTCHA_REQUIRED"
  | "XHS_LOGIN_REQUIRED"
  | "SESSION_EXPIRED"
...
export interface CloseoutHardStopRiskClassification {
  state: "clear" | "hard_stop";
  hard_stop: boolean;
  risk_class: CloseoutHardStopRiskClass | null;
  ...
  source:
    ...
    | "route_failure"
    | "none";
  required_recovery_action: string | null;
  should_block_route_action: boolean;
  evidence: {
...
type HardStopSource = Exclude<CloseoutHardStopRiskClassification["source"], "none">;

interface ReasonDefinition {
  riskClass: CloseoutHardStopRiskClass;
  recoveryAction: string;
  priority: number;
}

interface RiskSignal {
  reason: CloseoutHardStopRiskReason;
...
const REASON_DEFINITIONS: Record<CloseoutHardStopRiskReason, ReasonDefinition> = {
  ACCOUNT_ABNORMAL: {
...
const SOURCE_PRIORITY: Record<HardStopSource, number> = {
  account_safety: 50,
  route_failure: 40,
  api_response: 30,
  page_surface: 30,
...
const EXPLICIT_REASON_ALIASES: Record<string, CloseoutHardStopRiskReason> = {
...
const compact = (value: string | null): string => (value ?? "").replace(/\s+/gu, "");

const lower = (value: string | null): string => (value ?? "").toLowerCase();

const lowerCompact = (value: string | null): string => compact(value).toLowerCase();

const hasAny = (value: string, patterns: readonly string[]): boolean =>
  patterns.some((pattern) => value.includes(pattern));

const hasNarrativeMarker = (text: string): boolean => {
...
const explicitReason = (value: unknown): CloseoutHardStopRiskReason | null => {
...
const strongerSignal = (left: RiskSignal | null, right: RiskSignal | null): RiskSignal | null => {
  if (!left) {
    return right;
  }
  if (!right) {
...
const strongestSignal = (signals: RiskSignal[]): RiskSignal | null =>
  signals.reduce<RiskSignal | null>((current, item) => strongerSignal(current, item), null);
...
const textSignals = (
  ...
  if (
    normalized.includes("browser environment abnormal") ||
  ...
  if (
    normalized.includes("security redirect") ||
...
const titleSurfaceSignals = (value: string | null, source: HardStopSource): RiskSignal[] => {
  const raw = asString(value);
  if (!raw) {
    return [];
  }
...
const bodySurfaceSignals = (
  value: string | null,
  source: HardStopSource,
  allowByBlockingRoute: boolean
): RiskSignal[] => {
  const raw = asString(value);
  if (!raw) {
    return [];
  ...
  const hasBlockingAnchor =
    allowByBlockingRoute ||
    hasAny(text, [
      "平台拒绝当前请求",
      "当前访问存在安全风险",
      "验证后继续访问",
...
const pagePathSignals = (pageUrl: string | null): RiskSignal[] => {
  if (!pageUrl) {
    return [];
  }
  const path = (() => {
  ...
  if (
    segments.some((segment) => segment === "security-redirect" || segment === "security_redirect") ||
...
const buildClearClassification = (): CloseoutHardStopRiskClassification => ({
  state: "clear",
  hard_stop: false,
  risk_class: null,
  reason: null,
...
export const classifyCloseoutHardStopRisk = (input: {
  ...
  accountSafety?: unknown;
  accountSafetyFresh?: boolean;
  observabilitySignals?: unknown;
  pageSurface?: {
    url?: unknown;
    title?: unknown;
    bodyText?: unknown;
  ...
  const platformCode =
    asInteger(input.platformCode) ??
    asInteger(responseBody?.code) ??
    asInteger(accountSafety?.platform_code);

  const accountSafetySignals: RiskSignal[] = [];
  const accountSafetyReason = explicitReason(accountSafety?.reason);
  if (accountSafetyReason) {
    accountSafetySignals.push(
      signal(accountSafetyReason, "account_safety", asString(accountSafety?.reason))
    );
  ...
  const apiSurfaceSignals = (surface: {
    ...
    const surfacePlatformCode =
      asInteger(surface.platformCode) ??
      asInteger(surfaceBody?.code);
    const responseMessage =
      asString(surfaceBody?.msg) ??
