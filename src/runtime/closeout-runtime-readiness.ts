...
export type CloseoutRuntimeBlockerCode =
  | "runtime_not_ready"
  | "bootstrap_stale_unrecoverable"
  | "target_mismatch"
  | "lock_conflict"
  | "identity_mismatch"
...
export interface CloseoutRuntimeReadinessPreflight {
  ...
  target_binding: {
    requested: boolean;
    state: "not_requested" | "verified" | "missing" | "mismatch";
    requested_target_tab_id: number | null;
    requested_target_domain: string | null;
    requested_target_page: string | null;
    managed_target_tab_id: number | null;
    managed_target_domain: string | null;
    managed_target_page: string | null;
    target_tab_continuity: string | null;
  };
  runtime_status: {
  ...
  takeover_evidence: JsonObject | null;
}
...
const blocker = (
  blockerCode: CloseoutRuntimeBlockerCode,
  requiredRecoveryAction: string
): CloseoutRuntimeReadinessPreflight["blocker"] => ({
  blocker_layer: "runtime_readiness",
  blocker_code: blockerCode,
  required_recovery_action: requiredRecoveryAction
});

const buildTargetBinding = (
  params: JsonObject,
  takeoverEvidence: JsonObject | null
): CloseoutRuntimeReadinessPreflight["target_binding"] => {
  ...
  const requested =
    requestedTargetTabId !== null || requestedTargetDomain !== null || requestedTargetPage !== null;
  ...
  let state: CloseoutRuntimeReadinessPreflight["target_binding"]["state"] = "not_requested";
  if (requested) {
...
const hasStaleBootstrapRebindBaseEvidence = (input: {
  ...
  targetBinding: CloseoutRuntimeReadinessPreflight["target_binding"];
...
const hasStaleBootstrapRebindEvidence = (
  input: Parameters<typeof hasStaleBootstrapRebindBaseEvidence>[0] & {
    requestedAt: string | null;
  }
): boolean =>
  hasStaleBootstrapRebindBaseEvidence(input) &&
