...
当 PR 落入专项门禁，或其职责属于 formal spec review PR / live evidence 治理落库或治理维护 PR 时，PR 描述必须先显式提供结构化 `gate_applicability`，至少包含：
...
其中，治理相关 lane 只有在 reviewer / guardian 同时确认以下条件时才成立，不得只信作者自报 lane：
...
- `gate_applicability.governance_context_issue_ref=#310` 时，归入 `governance_landing_pr`
- `gate_applicability.governance_context_issue_ref` 非空且不等于 `#310` 时，归入 `governance_maintenance_pr`

若精确命中上述五处治理目标文件却缺少 `governance_context_issue_ref`，必须视为 `missing_governance_issue_ref` 直接阻断，不得退回普通 PR。

若携带治理 issue 引用的 PR 只命中上述目标文件子集，或在五处目标文件之外再夹带其他实质性改动，必须视为 `invalid_governance_landing_scope` 直接阻断，不得降格成 `general_pr`。

若同一 PR 同时命中 FR-0016 正式契约文件，或命中 `docs/dev/specs/FR-0016-live-evidence-governance-gate/TODO.md`，且又命中任一治理目标文件，必须视为 `mixed_spec_and_governance_scope` 直接阻断；不需要等到完整五文件治理形态才触发。
...
- 流程与元数据合规
  ...
  - 若事项需要 integration 联动，PR 描述是否完整提供 `integration_check`
...
- 对 formal spec review PR、治理落库/治理维护 PR 与所有落入“真实 Live Evidence 专项门禁”的 PR，必须先核对 `gate_applicability`
- 对治理落库/治理维护 PR，必须继续核对 `governance_context_issue_ref`、精确五文件治理范围、formal spec review 是否已通过，以及是否触发 `mixed_spec_and_governance_scope`；任一项不满足都应视为阻断
