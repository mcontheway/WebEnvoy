...
4. 正式 FR 套件必须至少包含 `spec.md`、`plan.md`、`TODO.md`；其余补充文档按复杂度按需添加。
   起草时应把“spec”理解为整套正式 FR 套件，而不是只写 `spec.md`。
   如事项引入稳定接口、共享数据模型、关键外部未知项或高风险链路，对应的 `contracts/`、`data-model.md`、`research.md`、`risks.md` 即转为必需，而不是可选装饰。
...
- spec review 与实现 PR 默认分离，不把正式 spec 变更与功能代码混在同一个 PR
- 正式 spec 变更的审查标准统一看 `spec_review.md`
- 如 PR 同时改动 `docs/dev/specs/**` 与实现代码，默认视为流程违规，除非明确属于纯 TODO/进度回写且不改变正式契约
- `spec.md` 的验收场景默认使用 `Given / When / Then`
- 起草正式套件时，必须补齐异常与边界场景，以及 `plan.md` 的最低 7 节：`实施目标`、`分阶段拆分`、`实现约束`、`测试与验证策略`、`TDD 范围`、`并行 / 串行关系`、`进入实现前条件`
